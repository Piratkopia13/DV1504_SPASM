...
TimeVisualization::TimeVisualization(Level* level, Gamemode* gamemode) {
	...
	m_timeBlocks.resize(int(m_timeLeft / 1.5f));
	...
	if (m_timeLeft < 60000000.f) {
		...
		m_middle = DirectX::SimpleMath::Vector3((m_level->getGridWidth() * m_level->DEFAULT_BLOCKSIZE) / 2.f - level->DEFAULT_BLOCKSIZE / 2.f, (m_level->getGridHeight() * m_level->DEFAULT_BLOCKSIZE) / 2.f - level->DEFAULT_BLOCKSIZE / 2.f, 3.f);
		for (int i = 0; i < m_timeBlocks.size(); i++) {
			m_timeBlocks[i] = std::make_unique<AnimatedObject>(m_blockModel);
			
			float x = cos(i * paj + PI / 2.f) * 2.5f * scale + m_middle.x;
			float y = sin(i * paj + PI / 2.f) * 2.5f * scale + m_middle.y;
			...
			m_timeBlocks[i]->setTargetPos(DirectX::SimpleMath::Vector3(x, y, 3.f * scale), 3.f);
