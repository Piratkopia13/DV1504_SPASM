...
DXTexture::DXTexture(const std::string& filename) {
	...
	ThrowIfFailed(Application::getInstance()->getDXManager()->getDevice()->CreateTexture2D(&texDesc, nullptr, &m_texture));
	// Fill the texture with data
	UINT rowPitch = sizeof(unsigned char) * data.getWidth() * 4;
	Application::getInstance()->getDXManager()->getDeviceContext()->UpdateSubresource(m_texture, 0, nullptr, data.getTextureData(), rowPitch, 0);
	...
	Application::getInstance()->getDXManager()->getDeviceContext()->GenerateMips(m_resourceView);
...
DXTexture::DXTexture(UINT width, UINT height, UINT aaSamples, UINT bindFlags) {
	...
	// Create the ShaderResourceView
	Application::getInstance()->getDXManager()->getDevice()->CreateShaderResourceView(m_texture, &srvDesc, &m_resourceView);
