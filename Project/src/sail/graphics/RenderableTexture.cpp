...
void RenderableTexture::createTextures() {
	...
	if (!m_onlyDSV) {
		...
		Memory::safeRelease(m_renderTargetView);
		// Create the new
		Application::getInstance()->getDXManager()->getDevice()->CreateRenderTargetView(m_dxColorTexture->getTexture2D(), &rtvDesc, &m_renderTargetView);
	}
	...
	if (m_hasDepthStencilView) {
		...
		Memory::safeRelease(m_depthStencilView);
		// Create the new
		Application::getInstance()->getDXManager()->getDevice()->CreateDepthStencilView(m_dxDepthTexture->getTexture2D(), &dsvDesc, &m_depthStencilView);
	}
	...
	if (m_aaSamples > 1) {
		...
		Application::getInstance()->getDXManager()->getDevice()->CreateTexture2D(&texDesc, nullptr, &m_nonMSAAColorTexture2D);
		...
		Application::getInstance()->getDXManager()->getDevice()->CreateShaderResourceView(m_nonMSAAColorTexture2D, &srvDesc, &m_nonMSAAColorSRV);
...
void RenderableTexture::end() {
	...
	if (m_aaSamples > 1)
		Application::getInstance()->getDXManager()->getDeviceContext()->ResolveSubresource(m_nonMSAAColorTexture2D, 0, m_dxColorTexture->getTexture2D(), 0, DXGI_FORMAT_R32G32B32A32_FLOAT);

	// Revert render target to the back buffer
	Application::getInstance()->getDXManager()->renderToBackBuffer();
}
